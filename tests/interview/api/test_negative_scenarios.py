...
class TestNegativeScenarios:
    ...
    def test_bad_ws_message_type(self, client, isolated_db):
        """Unknown WS message type returns error event."""
        interview_id = persist_interview_with_answers(
            Interview(
                id="neg-ws-1",
                locale="en",
        ...
        with client.websocket_connect(f"/interview/{interview_id}/theory/ws") as ws:
            ws.send_json({"type": "UNKNOWN_COMMAND_123"})
            err = ws.receive_json()
        assert err["type"] == "error"
        assert "Unknown" in err["message"]

    def test_audio_answer_invalid_wav(self, client, isolated_db, monkeypatch):
        """Bad WAV bytes → 503 when transcription service unavailable."""
        monkeypatch.setattr(
        ...
        response = client.post(
            ...
            data={"question_id": "q1"},
            files={"file": ("answer.wav", b"not-a-wav-file", "audio/wav")},
        )
    ...
    def test_audio_answer_wrong_content_type_returns_error(self, client, isolated_db, monkeypatch):
        ...
        monkeypatch.setattr(
            "app.speech.services.transcriber_resolver.resolve_speech_transcriber",
        ...
        response = client.post(
            f"/interview/{interview_id}/theory/audio-answer",
            data={"question_id": "q1"},
            # No files attached → missing required UploadFile
        )
